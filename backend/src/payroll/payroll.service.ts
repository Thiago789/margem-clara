...
export class PayrollService {
  ...
  async uploadMarginFile(
    ...
    if (existing) {
      assertMatchingFileRequest(existing, {
        idempotencyKey,
        payrollCycleId: cycleId,
        fileType: "MARGIN",
      ...
      return { ...this.toFileView(existing), duplicate: true };
    }
    ...
    try {
      ...
      if (isUniqueConflict(error)) {
        ...
        if (duplicate) {
          assertMatchingFileRequest(duplicate, {
            idempotencyKey,
            payrollCycleId: cycleId,
            fileType: "MARGIN",
            environment: metadata.environment,
          ...
          return { ...this.toFileView(duplicate), duplicate: true };
        }
      }
      throw error;
    }
  ...
  async generateInsertionFile(
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      await transaction.outboxEvent.create({ data: {
        ...
        eventType: "payroll.insertion_generated",
        payload: { payrollFileId: file.id, payrollCycleId: cycleId, protocolNumber, contractCount: generated.length },
        correlationId: context.correlationId,
      } });
      return { ...this.toFileView(file), duplicate: false };
    }, { isolationLevel: "Serializable" });
  }
